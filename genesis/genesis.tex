...
cosmology for field generation, emulation, and parameter inference.
However, diffusion-based approaches typically require many sampling
steps and provide likelihood information only through a
lower bound. Flow matching has recently emerged as an alternative
generative framework with stable training, efficient sampling, and
tractable likelihood evaluation, but its use for conditional multifield
cosmological emulation remains limited. This work presents \textsc{Genesis}, a conditional generative
emulator for multifield cosmological maps based on optimal-transport
flow matching. To date, flow matching has not been applied to
conditional multifield map generation.
The model jointly generates dark matter density, gas density, and gas temperature maps,
conditioned on the two cosmological parameters $(\Omega_{\rm m},\sigma_8)$ and four
astrophysical feedback parameters of CAMELS.
\textsc{Genesis} produces multifield samples that reproduce key CAMELS
summary statistics and preserve the main correlations among the three fields.
...
GPU.
As an initial demonstration of the likelihood capability of the
learned flow, a two-dimensional $\Omega_{\rm m}$--$\sigma_8$
scan is performed for one CAMELS map while fixing the remaining
parameters, and the relative likelihood is found to peak at the input
parameter values within the evaluated grid. These results indicate that
flow matching is a promising framework for fast
