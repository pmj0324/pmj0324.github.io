...
the stochastic trace estimate and summarizes the scan setup, and
...
\subsection{Trace estimation and scan setup}
...
ODE step, whose dimension is set by all map pixels and channels. This
is computationally prohibitive for the multifield maps considered here,
so the divergence is estimated stochastically using the Hutchinson
identity,
\begin{equation}
  {\rm Tr}\!\left[
    \frac{\partial u_t^\phi}{\partial \mathbf{x}}
  ...
  \label{eq:hutchinson}
\end{equation}
Rademacher probe vectors are used in the calculations below, and the
likelihood values are averaged over independent Hutchinson estimates to
reduce trace-estimator noise.
