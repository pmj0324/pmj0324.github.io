...
\experienceitem{GENESIS: Multi-Field Cosmological Simulation and Inference}{May 2025 -- Present}{Independent Research}{
    \begin{itemize}[leftmargin=*, itemsep=2pt]
        \item Designed GENESIS (GENerative AI for thE Simulation and Inference of cosmological fieldS) to connect two tasks in one flow-matching framework: conditional cosmological field generation and training-free parameter inference.
        \item The model learns a conditional probability-flow vector field $\mathbf{v}_\theta(\mathbf{x}_t,t,\mathbf{c})$ and integrates it to generate dark matter, gas, and temperature fields conditioned on CAMELS parameters.
        \item The same learned flow is also evaluated through the change-of-variables formula, enabling likelihood-based inference of cosmological parameters $\mathbf{c}$ without additional training.
    \end{itemize}
}
